...
import { registerAttendeeForOrganizer as manualRegistration, RegistrationEmailConflictError, EventRegistrationNotAllowedError } from "./register-attendee-for-organizer.js";

import { importRegistrationCsvForOrganizer } from "./import-registration-csv-for-organizer.js";
...
describe("concurrent CSV import", () => {
  ...
  it.each(["same organizer", "different organizers"])("keeps exactly one normalized-email winner: %s", async mode => {
    ...
    expect(await inspection.select().from(registrations).where(eq(registrations.eventId, event.id)))
      .toEqual([{ id: winner.id, eventId: event.id, attendeeId: winner.attendee.id, status: "confirmed", source: "csv",
  ...
  it.each(["same organizer", "different organizers"])("imports one complete overlapping reversed batch: %s", async mode => {
    const owner = actor(); const other = mode === "same organizer" ? owner : actor(); const event = await create(owner);
    if (mode === "different organizers") {
      const [user] = await inspection.insert(users).values({ externalSubject: subject(other) }).returning();
      await inspection.insert(eventStaff).values({ eventId: event.id, userId: user.id, role: "organizer" });
    }
    const shared = [input(), input()]; const exclusive = [input(), input()];
    const batches = [[exclusive[0], ...shared], [...shared].reverse().concat(exclusive[1])];
    const results = await Promise.allSettled(batches.map((rows, i) =>
      importRegistrationCsvForOrganizer(databases[i], event.id, csv(rows), i === 0 ? owner : other)));
    expect(results.filter(r => r.status === "fulfilled")).toHaveLength(1);
    expect(results.filter(r => r.status === "rejected")[0].reason).toBeInstanceOf(RegistrationEmailConflictError);
    const winner = results.findIndex(r => r.status === "fulfilled");
    expect(await inspection.select().from(registrations).where(eq(registrations.eventId, event.id))).toHaveLength(3);
    for (const r of batches[winner]) expect(await inspection.select().from(attendees).where(eq(attendees.email, r.email))).toHaveLength(1);
    expect(await inspection.select().from(attendees).where(eq(attendees.email, exclusive[1-winner].email))).toEqual([]);
  });
  it("coordinates a whole CSV batch with a concurrent manual registration", async () => {
    const owner = actor(); const event = await create(owner); const rows = [input(), input()];
    const results = await Promise.allSettled([
      importRegistrationCsvForOrganizer(databases[0], event.id, csv(rows), owner),
      manualRegistration(databases[1], event.id, rows[1], owner),
    ]);
    expect(results.filter(r => r.status === "fulfilled")).toHaveLength(1);
    expect(results.filter(r => r.status === "rejected")[0].reason).toBeInstanceOf(RegistrationEmailConflictError);
    const csvWon = results[0].status === "fulfilled";
    expect(await inspection.select().from(registrations).where(eq(registrations.eventId, event.id))).toHaveLength(csvWon ? 2 : 1);
    expect(await inspection.select().from(attendees).where(eq(attendees.email, rows[0].email))).toHaveLength(csvWon ? 1 : 0);
    expect(await inspection.select().from(attendees).where(eq(attendees.email, rows[1].email))).toHaveLength(1);
  });
});
