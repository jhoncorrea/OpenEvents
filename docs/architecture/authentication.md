...
## Autorización para emitir credenciales (#63)

La operación interna requiere organizer global, identidad tenantId/objectId válida, usuario local activo y event_staff con rol organizer para el evento. Resuelve external_subject como entra:<tenantId>:<objectId>, normalizado; no usa correo ni el subject libre para identificar al usuario. No provisiona usuarios ni concede acceso implícito a admin/checkin_operator. Con varios roles globales, debe seguir existiendo la asignación local organizer.

El usuario, la asignación y el evento se leen con bloqueos SHARE; la inscripción, con UPDATE, dentro de la transacción. Una baja o revocación confirmada antes de obtener el bloqueo se observa y deniega la emisión; si la emisión obtiene primero los bloqueos, el cambio concurrente espera a que termine. Esto no revoca automáticamente una credencial ya emitida. El adaptador HTTP de #65 autentica al actor mediante el guard Bearer existente antes de invocar esta función. Contrato: [Credenciales](registration-credentials.md).
