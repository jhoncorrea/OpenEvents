...
### Importación y recuperación CSV desde la web - OE-03-002E (Issue #51)

Desde Mis eventos, el organizador con acceso comprobado abre Importar CSV. Se consulta de nuevo el evento antes de abrir y al volver. La API conserva la autorización definitiva. En draft/active se puede comenzar una importación; en closed/cancelled se ofrece recuperación de la operación guardada, sin crear una nueva.

El cliente envía bytes originales con Bearer, Idempotency-Key y text/csv. Comprueba archivo no vacío y máximo 1 MiB antes del envío. La API valida UTF-8, estructura, 500 registros y conflictos. La pantalla no confía en extensión/MIME del archivo como prueba de validez. Recibe hasta 100 diagnósticos y muestra etiquetas propias, no mensajes arbitrarios del servidor.

Cada respuesta completed se valida antes de mostrarla: UUID, evento, fechas UTC, cantidad de 1 a 500, items confirmed/csv, datos normalizados y ausencia de IDs/correos duplicados. Se proyectan campos explícitos. HTTP 200 not_observed solo es válido para GET y no significa fallo. Un error de transporte, timeout de 15 segundos de HTTP/lectura, JSON incoherente o código desconocido mantiene incertidumbre. No hay reintentos ni redirecciones automáticas. La espera de MSAL queda fuera del timeout HTTP.

Antes del primer POST se guarda en sessionStorage una entrada por cuenta/evento con solo key UUID y hash SHA-256. No se guardan CSV, nombre de archivo, comprobante ni tokens adicionales por este módulo (la caché MSAL existente conserva su política). Fallos de almacenamiento o metadata corrupta bloquean envíos nuevos; no se borran silenciosamente para continuar. El archivo permanece en memoria mientras la pantalla está abierta.

Tras recarga, navegación o nueva autorización en la misma pestaña, se puede consultar con la clave guardada sin archivo. Para reenviar se debe seleccionar un archivo cuya huella coincida exactamente; un cambio de BOM o saltos de línea cambia la huella. Los errores definitivos de validación/conflicto de correo/estado del primer intento permiten corregir; si ya existía incertidumbre, no la elimina un error posterior. Nueva importación requiere resultado completed y acción explícita. No hay botón para descartar una operación incierta y generar otra clave.

Cambio de cuenta/evento, cierre de sesión o interacción MSAL desmontan la pantalla, abortan solicitudes y descartan resultados tardíos. La metadata permanece aislada por cuenta/evento para regresar con la misma cuenta; conocer una clave no concede permisos. Una señal beforeunload advierte mientras hay trabajo pendiente, sin garantía de que todo navegador muestre el diálogo. Cerrar la pestaña o borrar sessionStorage pierde la recuperación local: conservar el archivo y la clave mostrada, y solicitar verificación antes de iniciar otra operación. No existe recuperación automática entre dispositivos ni formulario para introducir claves externas en este incremento.

El comprobante es histórico; el estado actual se consulta en Ver inscripciones. 401/403 y fallos de autenticación invalidan el acceso comprobado; 404 retira el evento. Se bloquean acciones simultáneas, se enfoca el encabezado y se usan etiquetas y mensajes accesibles. No hay cambios de API, migraciones ni dependencias.

Verificación del agente en copia aislada: typecheck y lint aprobados; 152 pruebas focalizadas aprobadas (36 cliente CSV, 3 metadata, 17 pantalla, 44 MyEvents y 52 SessionControls). Son 63 casos nuevos; los anteriores son regresiones y no se suman aparte. Validación global confirmada por la salida del mantenedor del 21 de septiembre de 2026: 1.595 pruebas aprobadas (602 API, 698 web y 295 de integración PostgreSQL), typecheck, lint y build correctos. Las 152 focalizadas están incluidas y no se suman de nuevo. git diff --check sin errores; solo avisos CRLF a LF. Evidencia manual: importación confirmada de dos inscripciones y recuperación del mismo comprobante, con igual identificador, fecha y cantidad, siguiendo el recorrido de recarga en la misma pestaña, cuenta y evento. Esta comprobación no cubre un corte de red real ni un cambio de cuenta. Pendientes commit, PR, CI y merge. No se declara completo RF-ATT-002 sin contrastar sus criterios completos.
